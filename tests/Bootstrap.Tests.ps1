...
Describe 'Bootstrap — bundle integrity (F1 / issue #8)' {
    ...
    It 'a bundle built with a RELATIVE -OutputDir passes its own integrity check' {
        ...
        $cacheDir = Join-Path $Root 'src/helpers/__pycache__'
        $cacheFile = Join-Path $cacheDir 'mts_bundle_regression.pyc'
        New-Item -ItemType Directory -Path $tmp -Force | Out-Null
        New-Item -ItemType Directory -Path $cacheDir -Force | Out-Null
        Set-Content -LiteralPath $cacheFile -Value 'host-generated bytecode' -Encoding ascii
        Push-Location $tmp
        try {
            & (Join-Path $Root 'bundle/build-bundle.ps1') -OutputDir 'rel' -Version 'reltest' -SkipPwsh -SkipVenv -SkipKev *> $null
            ...
            Test-Path -LiteralPath (Join-Path $bundle 'src/helpers/__pycache__') | Should -BeFalse
            @($man.fileHashes.PSObject.Properties.Name | Where-Object { $_ -match '(?i)(^|/)__pycache__/|\.py[co]$' }).Count | Should -Be 0
        } finally {
            Pop-Location
    ...
    It 'builds a ZIP when a portable runtime archive is supplied' {
        # Regression: the local `$zip` path variable collided case-insensitively
        # with the [switch]$Zip parameter, so every production ZIP build failed
        # before the portable runtime could be expanded.
        $tmp = Join-Path $env:TEMP "mts-zipbuild-$(Get-Random)"
        $runtimeDir = Join-Path $tmp 'runtime'
        $outputDir = Join-Path $tmp 'out'
        New-Item -ItemType Directory -Path $runtimeDir,$outputDir -Force | Out-Null
        Set-Content -LiteralPath (Join-Path $runtimeDir 'pwsh.exe') -Value 'test runtime' -Encoding ascii
        $runtimeZip = Join-Path $tmp 'runtime.zip'
        Compress-Archive -Path (Join-Path $runtimeDir '*') -DestinationPath $runtimeZip
        try {
            & (Join-Path $Root 'bundle/build-bundle.ps1') -OutputDir $outputDir -Version 'ziptest' `
                -PwshVersion '7.4.19' -PwshZip $runtimeZip -SkipVenv -SkipKev -Zip *> $null
            Test-Path -LiteralPath (Join-Path $outputDir 'media-transfer-scan-tool-ziptest.zip') | Should -BeTrue
            $manifest = Get-Content (Join-Path $outputDir 'media-transfer-scan-tool-ziptest/manifest.json') -Raw | ConvertFrom-Json
            $manifest.bundleVersion | Should -Be 'ziptest'
            $manifest.toolVersions.pwsh | Should -Be '7.4.19'
        } finally {
            Remove-Item $tmp -Recurse -Force -ErrorAction SilentlyContinue
        }
    }
