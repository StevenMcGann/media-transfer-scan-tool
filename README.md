...
> **Status:** **v0.17.0** is the current release. It removes a Microsoft Defender false positive against the scanner itself: PowerShell threat indicators are now matched by SHA-256 token identity instead of being rebuilt from text fragments, with BOM-aware decoding and bounded, fail-closed rule scanning. Every originally planned file-type family remains implemented: Python, disguised scripts, Office/PDF, shell, PowerShell, VBA/VBScript, npm, NuGet, native binaries, ML/model files, and recursively inspected archives.
>
