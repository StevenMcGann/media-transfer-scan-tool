...
- **Nested:** archive members are re-classified recursively after extraction (issue #31 ✅ — `Invoke-ArchiveMemberDispatch`, Engine.ps1), subject to a depth cap and a cumulative member-count/byte budget shared across the whole scan run (not just per archive — see §4 archive hardening for the per-archive bomb caps this sits alongside).
...
| **npm** *(v0.6 ✅)* | `.tgz`, `package.json`, `.js .mjs .cjs .ts` | yes (tar) | JS risky patterns (child_process/eval/Function/obfuscation) | detect-secrets | **OSV** (api.osv.dev) vs `package-lock.json` exact versions (online, via `OsvScan` — issue #32) | — | **pre/post/install lifecycle scripts** (top signal, HIGH) + risky-command inspection, `bin` shims; pure-PowerShell core (no Node) |
| **NuGet** *(issue #32 ✅)* | `.nupkg` | yes (ZIP, hardened path) | — | — | **OSV** (api.osv.dev) vs the package's own id/version, read from its embedded `.nuspec` (online) | — | The artifact itself is the pinned dependency — no separate lock file to read |
...
| **Generic archive** *(v0.8 ✅, recursive member dispatch issue #31 ✅)* | `.zip .whl .egg`, `.tgz .tar.gz` | yes (hardened) | via member dispatch | via member dispatch | via member dispatch | via member dispatch | Pre-extraction hazard inspection: **zip-slip / path-traversal HARD-BLOCK**, **decompression-bomb caps** (per-entry ratio + 512 MB aggregate + entry-count), **symlink** flag, **nested-archive** flag; tar gets a traversal pre-check + Python `data` filter. Every extracted member is then classified by content (not extension — catches a disguised script for free) and dispatched through the FULL analyzer set, findings folded onto the parent under the `archive!inner/path` label — not added as new top-level units. A nested archive is extracted and recursed the same hardened way, subject to a depth cap and a cumulative member-count/byte budget **shared across the whole scan run** (not reset per archive). Semantic containers (wheels/eggs, `.nupkg`, PyTorch `.pt/.pth`) are extracted but NOT member-dispatched — their own whole-staging-tree analyzer already covers them; recursing further would duplicate those findings. |
