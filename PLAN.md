...
| `UnitTypes` | string[] | Types it handles: `python`,`npm`,`powershell`,`shell`,`office`,`pdf`,`model`,`disguised`,`archive`, or `any`. |
...
| **Disguised** *(v0.2, first new)* | any ext / no ext | n/a | reroute to matched analyzer | detect-secrets | — | — | shebang + magic-byte + signature sniff; **extension/content mismatch** finding |
...
| **Model / LLM** *(v0.7)* | `.pkl .pickle .pt .pth .bin .joblib .h5 .pb .onnx .safetensors .gguf` | maybe | — | detect-secrets on sidecar configs | — | **pickle opcode scan** (flag `GLOBAL`/`REDUCE`/`STACK_GLOBAL` → arbitrary code on load) | recognize safe-by-design formats (safetensors/gguf) vs dangerous (pickle/torch); scan config JSON/YAML for URLs & endpoints |
| **Generic archive** *(v0.8)* | `.zip .tar .7z .gz` (non-package) | yes | route contents recursively | — | — | — | **zip-slip / path traversal**, decompression-bomb caps (ratio + absolute size), symlink entries, nested-archive depth limit |
